    ...
    <div class="card">
        <thead>
          <tr>
            ...
            <th scope="col">Diseas Name</th>
            <th scope="col">Diseas Image</th>
            <th scope="col">Action</th>
          </tr>
        </thead>
      <div class="card-body ">

         @foreach ($diseas as $key=> $diseas)
         <tbody>
            <td>{{ $key+1 }}</td>
            <td>{{ $diseas->diseas_name }}</td>
            <hr>
            <td><img width="100" src="{{ asset('diseas') }}/{{ $diseas->diseas_image }}" alt=""></td>
          
           <td>
            <a href="{{ route('edit.diseas', $diseas->id) }}" class="btn btn-primary shadow btn-xs sharp mr-1"><i class="fa fa-pencil"></i></a>
              <a href="{{ route('delete.diseas', $diseas->id) }}" class="btn btn-danger shadow btn-xs sharp" id="delete"><i class="fa fa-trash"></i></a>
            </td>
          </td>
